...
`experiments/v7_mrmp_prepare.py` が、clone 済み MRMP から次を生成する（既定出力は **`experiments/logs/mrmp_prepared/`**・git 管理外）。

| 成果物 | 内容 |
|--------|------|
| `windows.jsonl` | 発話ごとに **直近 W ターン**を `話者ID: 本文` で連結した `text`（HF 前向き用）。**P1**: `speaker_src`＝直前話者。 |
| `dialogue_eval.jsonl` | 対話×話者ごとの MRMP 評価（1–5 を **0–1** に正規化した `mrmp_*_01`） |
| `manifest.json` | 件数・`window_turns`・`pair_rule` |

```bash
python experiments/v7_mrmp_prepare.py --corpus-root experiments/logs/mrmp_repo/multi_relational_multi_party_chat_corpus --window 4
```

整形直後の行数・必須フィールド・`manifest.json` との整合は  
`python experiments/rvt_exp_2026_008_day0_checks.py --strict-manifest`  
で確認できる（[RVT-EXP-2026-008 架け橋 §4](rvt_exp_2026_008_architecture_bridge.md)）。

相関（MRMP ラベル × `||S_asym||_F`）は `v7_phase1a_pilot_jsonl.py` の **`--mrmp-labels`**（大規模時は **`--max-rows`**）。

```bash
python experiments/v7_phase1a_pilot_jsonl.py --demo --mrmp-labels \
  --jsonl experiments/data/v7_mrmp_sample.jsonl
```

コミット用の先頭 3 行サンプル: [`experiments/data/v7_mrmp_sample.jsonl`](../../experiments/data/v7_mrmp_sample.jsonl)。
