...
### Phase 4 — 大規模ノード・分散（Jetson 等）

**位置づけ**: 和文の**独立ロードマップ** [ROADMAP_Phase4_分散とエッジ_ja.md](ROADMAP_Phase4_分散とエッジ_ja.md)（チェックリスト・追跡用）。ルートの [ROADMAP_ja.md](../../ROADMAP_ja.md) は Phase 1A〜3 を中心とし、Phase 4 は当該文書へ委譲する。上記 **Phase A〜C**（単一プロセス・単一ノード中心の技術実証）とは**切り離した**フェーズとして扱う。

- **対象**: エッジデバイス群・クラスタ上での共鳴場の分割・同期・観測（例: Jetson Orin ノード、センサ連携）。本リポジトリの既定 CI や Phase A〜C の完了条件には**含めない**。
- **参照**: [2 週間軽量プロトタイプ計画](../tutorials/getting_started.md) の 2nd Stage（Jetson クラスター記述）、[実装基本設計](../implementation/architecture_overview.md) との将来接続を想定した設計メモ。

Phase C（二階建て・HBM）が「単一デコーダスタック上の資源最適化」に主眼があるのに対し、Phase 4 は**空間的・ノード間の分散**が主題である。
...
**Phase 4**（大規模ノード・分散／Jetson 等）は上表の A〜C とは**別軸**。[ROADMAP_Phase4_分散とエッジ_ja.md](ROADMAP_Phase4_分散とエッジ_ja.md) を参照。
...
| 2026-03-25 | Phase 4（README／ROADMAP_ja）: 大規模ノード・分散（Jetson 等）を Phase A〜C から分離 |
| 2026-03-25 | Phase 4 の和文チェックリストを [ROADMAP_Phase4_分散とエッジ_ja.md](ROADMAP_Phase4_分散とエッジ_ja.md) へ分離（独立ロードマップ化） |
