...
| V_K（一般） | **現状**: ½‖W‖²_F・W\*=0 のスタブ（`simulate_tau_v_k_series`） | **予定**: 理論の項・W_D\*・Krasovskii 第2項を段階導入 |
...
3. **V_K 本実装**（項の追加順・W\* の定義）を小さなモジュールで段階導入 → テストと JSON スキーマ `v7_phase2a_*.v1` を分離。
4. **τ*_exp 操作定義**を設計書に沿って文書化し、ログフィールドを固定。
