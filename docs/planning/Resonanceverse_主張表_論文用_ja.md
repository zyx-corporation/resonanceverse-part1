...
**品質 τ の照合:** T2（因果 LM ppl）は従来記録のまま**未達**。T3（GLUE SST-2）では**v0.2（T3′）**で **baseline** は τ_acc を満たし、**awai・読み出し `narrow`** は未達であった。**v0.3（T3″）**として**同一学習条件**で **`--awai-readout projected` または `dual`** を採用した awai も **τ_acc を満たす**記録を得た（下表・要約 JSON）。**ゲート上は** baseline／awai（T3″）のいずれも τ に届くが、**数値の完全一致や二階建て一般の優位**は主張しない。T4（SQuAD EM）は**v0.2（T4′）で τ_em を満たす**（v0.1 極短学習は未達のまま併記）。資源比較と品質ゲートは**分離**して報告する。

**複数ノード・エッジ配備**（分散共鳴場の同期・観測）は、本リポジトリの Phase A〜C の**必須経路ではなく**、別ロードマップとして位置づける（[ROADMAP Phase 4](ROADMAP_Phase4_分散とエッジ_ja.md) 内「固定プロトコル草案（v0.1）」）。単一ホスト上の **IPC 往復スモーク**（[`distributed_sync_smoke_sample_v1.json`](../../experiments/baselines/distributed_sync_smoke_sample_v1.json)）は**オーダー感の記録**であり、分散レイテンシの代替ではない。
...
|----|------------|------|-------------------|------------------|
| **C-T2** | 「等品質」として報告する場合、**perplexity** が事前登録上限以下である | **記録済・τ 未達** | [`slm_resonance_lm_tau_t2_wikitext_gpt2_v1.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_wikitext_gpt2_v1.json)、[`phase2_tau_gate_summary_v1.json`](../../experiments/baselines/phase2_tau_gate_summary_v1.json) | τ_ppl = **50.0**（記録 ppl_eval ≈ **5.12×10⁴**） |
| **C-T3** | 二値下流で「等品質」とする場合、**検証 accuracy** が事前登録下限以上 | **記録済**（v0.1 凍結: 両系**未達**／T3′: baseline **達成**・awai **`narrow` 未達**／**T3″: awai `projected`・`dual` 達成**） | T3″: [`slm_downstream_tau_t3_sst2_awai_projected_prereg_v03.json`](../../experiments/baselines/slm_downstream_tau_t3_sst2_awai_projected_prereg_v03.json)、[`slm_downstream_tau_t3_sst2_awai_dual_prereg_v03.json`](../../experiments/baselines/slm_downstream_tau_t3_sst2_awai_dual_prereg_v03.json)、T3′: 上記 baseline／awai narrow、[`phase2_tau_gate_summary_v1.json`](../../experiments/baselines/phase2_tau_gate_summary_v1.json) | τ_acc = **0.55**（T3″: projected ≈ **0.839**、dual ≈ **0.847**） |
...
| **C-dist** | 複数ノード・エッジ上での共鳴場分割・同期の**固定プロトコル** | **草案**（単一ホスト IPC スモークのみ記録） | [ROADMAP_Phase4_分散とエッジ_ja.md](ROADMAP_Phase4_分散とエッジ_ja.md)、[`distributed_sync_smoke_sample_v1.json`](../../experiments/baselines/distributed_sync_smoke_sample_v1.json) | — |
