...
|----|------------|------|-------------------|------------------|
| **C-T2** | 「等品質」として報告する場合、**perplexity** が事前登録上限以下である | **記録済・τ 未達**（awai: **H+6 ヘッド**再計測でも未達；旧 6→V は参考） | [`slm_resonance_lm_tau_t2_prime_prereg_v05_awai_head_dual.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_prime_prereg_v05_awai_head_dual.json)、baseline-hf: [`…_baseline_hf.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_prime_prereg_v04_baseline_hf.json)、[`phase2_tau_gate_summary_v1.json`](../../experiments/baselines/phase2_tau_gate_summary_v1.json) | τ_ppl = **50.0**（v05 awai ≈ **3.7×10³**、baseline-hf ≈ **69**） |
...
| **C-res** | Router **`step_stride`**（決定的）のログ | **デモ + gpt2 実モデル 記録済** | デモ: [`two_tier_sweep_stride4_demo_summary_v1.json`](../../experiments/baselines/two_tier_sweep_stride4_demo_summary_v1.json)；**gpt2・CPU**（スイープのみ）: [`two_tier_stride4_gpt2_cpu_full.json`](../../experiments/baselines/two_tier_stride4_gpt2_cpu_full.json)；**gpt2・CPU・P0 同梱**: [`phase3_claim_gpt2_cpu_stride4_summary_v1.json`](../../experiments/baselines/phase3_claim_gpt2_cpu_stride4_summary_v1.json)；**gpt2・MPS・P0 同梱**: [`phase3_claim_gpt2_mps_stride4_summary_v1.json`](../../experiments/baselines/phase3_claim_gpt2_mps_stride4_summary_v1.json) | — |
...
**T2:** **v0.1** および **T2′（v0.4）**の **awai** は旧 **`AwaiIntegratedSLM`**（語彙ヘッドが **6→V のみ**）のため検証 ppl が極大（[`…_v04_awai.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_prime_prereg_v04_awai.json)）。**実装を隠れ状態＋共鳴の連結（H+6→V）に変更**後の **再計測**（[`slm_resonance_lm_tau_t2_prime_prereg_v05_awai_head_dual.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_prime_prereg_v05_awai_head_dual.json)、T2′と同一 200 steps 条件）では **ppl_eval が約 3.7×10³** まで改善したが **τ_ppl=50 未達**。**baseline-hf**（[`…_baseline_hf.json`](../../experiments/baselines/slm_resonance_lm_tau_t2_prime_prereg_v04_baseline_hf.json)）は **約 69** でも τ 未達。因果 LM の等品質主張には用いない（[phase_c_quality_tau_prereg.md](../api/modules/phase_c_quality_tau_prereg.md) 実装注記）。
...
| 2026-03-25 | **AwaiIntegratedSLM** 語彙ヘッドを **H+6 連結**に変更、T2 再計測（v05）、主張表・τ 文書・要約 JSON 更新 |
