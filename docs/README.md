...
- **RVT-EXP-2026-008（外部計画と本リポの対応・Day 0）**: [rvt_exp_2026_008_architecture_bridge.md](planning/rvt_exp_2026_008_architecture_bridge.md)（§4 に Phase II-A 任意連携 **`RVT_EXPLORE`**）、[`rvt_exp_2026_008_day0_checks.py`](../experiments/rvt_exp_2026_008_day0_checks.py)。**ワンショット探索**は [`run_rvt_explore.sh`](../experiments/run_rvt_explore.sh)（`windows.jsonl` が無いときは exit 0）、MRMP バッチは [`run_rvt_mrmp_batch.sh`](../experiments/run_rvt_mrmp_batch.sh)、L2 スモークは [`run_rvt_l2_smoke.sh`](../experiments/run_rvt_l2_smoke.sh)。
...
| [Phase II-A・定理支持の推奨経路](planning/v7_phase2a_theorem_support_path_ja.md) | 数値的支持の優先順位・チェックリスト・全体実装の横断事項 |
