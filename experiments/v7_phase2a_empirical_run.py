...
注意行列: v7_phase1a_phi_correlation.hf_forward_attention_layer_matrix
"""
...
from experiments.v7_phase1a_phi_correlation import hf_forward_attention_layer_matrix  # noqa: E402
...
def _load_rows(path: Path, offset: int, max_rows: int | None) -> list[dict[str, Any]]:
    from experiments.jsonl_slice import iter_jsonl_slice

    return list(iter_jsonl_slice(path, offset=offset, max_rows=max_rows))
...
def main() -> None:
    ...
    if not path.is_file():
        print(json.dumps({"error": "jsonl_not_found", "path": str(path)}), file=sys.stderr)
        sys.exit(1)
    ...
    for did in dialogue_ids:
        drows = by_d[did]
        drows.sort(key=lambda r: (int(r.get("utterance_id", 0)), str(r.get("id", ""))))
        ...
        for t_idx, row in enumerate(drows):
            ...
            A, err, _ntok = hf_forward_attention_layer_matrix(
                model=model,
                tokenizer=tok,
                device=device,
                text=text,
    ...
    by_tau = R_tau_equal_weight_per_dialogue(series_by_dialogue, tau_max=max_tau)
    ...
    if series_has_auxiliary_label_scores(series_by_dialogue):
        payload["auxiliary_label_delay_coherence"] = auxiliary_label_delay_coherence_by_axis(
            series_by_dialogue,
            tau_max=max_tau,
        )
    ...
    print(
        "v7_phase2a_empirical_run_ok",
        json.dumps({"taus": len(by_tau), "dialogues": len(series_by_dialogue)}, ensure_ascii=False),
    )
